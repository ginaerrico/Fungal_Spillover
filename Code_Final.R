...
#specific outcome for equal interspecific competition: coexistence      a21= 0.041, a12= 0.039,
plot(out0[,1], out0[,2],col="darkgreen",type="l",lwd=2,ylim=c(0,6))
points(out0[,1], out0[,3],col="palegreen2",type="l",lwd=2)
points(out0[,1], out0[,4],col="darkblue",type="l",lwd=2)

#specific outcome for equal P2 0.48, P1 0.38 -> P2 wins
Pars[5]<-0.38
Pars[6]<-0.48
out0 <- ode(y = State, func = model0,
                times = Time, parms = Pars)
...
#specific outcome for equal interspecific competition
 Pars <- c(
      alpha1= 3,
      alpha2= 3,
      a11= 0.5,
...
out01 <- ode.1D(y = State, func = model1,
                    times = Time, parms = Pars, nspec = 4,
                    names = c("x1","x2","y1","y2"), dimens = N)
plot(out01[,1], out01[,2],col="darkgreen",type="l",lwd=2,ylim=c(0,5.6))
points(out01[,1], out01[,52],col="palegreen2",type="l",lwd=2)
points(out01[,1], out01[,102],col="darkblue",type="l",lwd=2)
